...
from droneinterfaces.msg import HumanPoseCoor
import sys
import torch
import os
...
class HumanPose(Node):
    def __init__(self, name, predicter):
        ...
        self.subscription
        self.publisher = self.create_publisher(HumanPoseCoor, name+"_HumanPose", 10)
        self.coor = HumanPoseCoor()
        self.index = {"head":9, "neck":8, "back":7, "lshoulder":12, "rshoulder":13}
    ...
    def framecallback(self, msg):
        ...
        for i, key in enumerate(self.index.keys()):
            self.coor.coordinate[i*2] = humans[self.index[key]][0]
            self.coor.coordinate[i*2+1] = humans[self.index[key]][1]
